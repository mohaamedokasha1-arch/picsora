...
| `NEXT_PUBLIC_CONTACT_ENDPOINT` | Optional URL that receives the contact form as JSON `POST {name, email, message, reason}` (+ `company, teamSize, monthlyVolume, need` for business / custom-solution inquiries). When unset the form falls back to a prefilled email to `piclizer@gmail.com` opened in the visitor's mail app. If set, add its origin to `connect-src` in `lib/security/headers.mjs` |
